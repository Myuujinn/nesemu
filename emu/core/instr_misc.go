...
func (c *Cpu) AbsoluteBit(address []byte) (incrPC bool, cycles byte, debug string) {
	result := *c.Memory.Map(uint16(address[0]) | uint16(address[1])<<8)

	c.SetFlag(FlagZero, result&c.registers.acc == 0)
	c.SetFlag(FlagNegative, result&0x80 != 0)
	c.SetFlag(FlagOverflow, result&0x40 != 0)

	return true, 4, fmt.Sprintf("BIT $%02X%02X = %02X", address[1], address[0], result)
}
